...
class StockModel:
    ...
    def load_data(self):
        self.all_data = yf.download(["NVDA",
                                     "META",
                                     "RCL",
                                     "BLDR",
                                     "UBER",
    ...
    def plotting(self):
        ...
        iqr = q3 - q1
        temp_df = self.filter_data[~((self.filter_data[self.ticker][self.value] < q1 - 1.5 * iqr) | (
            self.filter_data[self.ticker][self.value] > q3 + 1.5 * iqr))]
        if len(self.value) == 2:
            return temp_df[self.ticker][self.value].plot(kind=self.graphtype,
                                                         xlabel='date',
                                                         ylabel=f"{self.value[0]} and {self.value[1]}",
                                                         title=f'{self.value[0]} vs {self.value[1]} chart of {self.ticker}',
                                                         ax=self.ax,
                                                         grid=True,
                                                         logy=False)
        ...
        return temp_df[self.ticker][self.value].plot(kind=self.graphtype,
                                                     xlabel='date',
                                                     ylabel=self.value[0],
                                                     title=f'{self.value[0]} chart of {self.ticker}',
                                                     ax=self.ax,
                                                     grid=True,
                                                     logy=False)
    ...
    def ploting_corr(self):
        ...
        return filter_data1.plot(
            kind="scatter",
            x=self.__corr1,
            y=self.__corr2,
            xlabel=f"{self.__corr1}",
            ylabel=f"{self.__corr2}",
            ax=self.ax_corr,
            title=f"Correlation between {self.__corr1} and {self.__corr2} in {self.__stock_cor}")
    ...
    def distribution_graph(self, stock, ax):
        ...
        volume_dist = temp_df["Volume"].plot(
            kind="hist",
            ax=ax,
            title=f"The volume distribution of {stock}",
            xlabel="The sales of volume (in ten million)")
        return volume_dist
    ...
    def All_prices(self, ax):
        ...
        return filter_data[att_list].plot(
            ylabel="Price",
            xlabel="Date",
            title="Stock Prices for NVDA",
            kind="line",
            ax=ax)
    ...
    def closing_volume(self, ax):
        ...
        return filter_data1.plot(
            ...
            x="Close",
            y="Volume",
            title="Correlation between Closing price and Volume",
            ax=ax)
